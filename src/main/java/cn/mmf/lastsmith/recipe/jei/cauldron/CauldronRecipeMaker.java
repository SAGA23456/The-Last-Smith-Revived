...
public class CauldronRecipeMaker {
    ...
    public static List<BrewingRecipeWrapper> getCauldronRecipes(IIngredientRegistry ingredientRegistry) {
        CauldronRecipeMaker cauldronRecipeMaker = new CauldronRecipeMaker(ingredientRegistry);
        return cauldronRecipeMaker.getCauldronRecipes();
    }

    private CauldronRecipeMaker(IIngredientRegistry ingredientRegistry) {
        this.ingredientRegistry = ingredientRegistry;
    }

    private List<BrewingRecipeWrapper> getCauldronRecipes() {
        ...
        brewingRecipes.stream().filter((r) -> r instanceof VanillaBrewingRecipe).map((r) -> (VanillaBrewingRecipe)r).findFirst().ifPresent((vanillaBrewingRecipe) -> this.addVanillaBrewingRecipes(recipes, vanillaBrewingRecipe));
    ...
    private void addVanillaBrewingRecipes(Collection<BrewingRecipeWrapper> recipes, VanillaBrewingRecipe vanillaBrewingRecipe) {
        ...
        do {
            List<ItemStack> newPotions = this.getNewPotions(knownPotions, potionIngredients, recipes, vanillaBrewingRecipe);
        ...
        for(ItemStack potionInput : knownPotions) {
            for(ItemStack potionIngredient : potionIngredients) {
                ...
                if (!potionOutput.isEmpty()) {
                    ...
                    if (!recipes.contains(recipe) && !this.disabledRecipes.contains(recipe)) {
