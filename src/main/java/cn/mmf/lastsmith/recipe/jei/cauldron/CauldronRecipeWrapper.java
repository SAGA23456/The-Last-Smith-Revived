...
public class CauldronRecipeWrapper implements IRecipeWrapper {
    ...
    public CauldronRecipeWrapper(List<ItemStack> ingredients, ItemStack potionInput, ItemStack potionOutput) {
        ...
        this.hashCode = Objects.hashCode(potionInput.getItem(), ForgeRegistries.POTION_TYPES.getKey(typeIn), potionOutput.getItem(), ForgeRegistries.POTION_TYPES.getKey(typeOut), firstIngredient.getItem(), firstIngredient.getMetadata());
    }

    public void getIngredients(IIngredients ingredients) {
        ingredients.setInputLists(VanillaTypes.ITEM, this.inputs);
        ingredients.setOutput(VanillaTypes.ITEM, this.potionOutput);
    }
    ...
    public void drawInfo(Minecraft minecraft, int recipeWidth, int recipeHeight, int mouseX, int mouseY) {
        ...
        if (brewingSteps < Integer.MAX_VALUE) {
            String steps = Translator.translateToLocalFormatted("gui.jei.category.brewing.steps", new Object[]{brewingSteps});
    ...
    public boolean equals(Object obj) {
        if (!(obj instanceof CauldronRecipeWrapper)) {
            return false;
        } else {
            CauldronRecipeWrapper other = (CauldronRecipeWrapper)obj;
            if (!arePotionsEqual(other.potionInput, this.potionInput)) {
                return false;
            } else if (!arePotionsEqual(other.potionOutput, this.potionOutput)) {
                return false;
            } else if (this.ingredients.size() != other.ingredients.size()) {
                return false;
            } else {
                for(int i = 0; i < this.ingredients.size(); ++i) {
                    if (!ItemStack.areItemStacksEqual(this.ingredients.get(i), other.ingredients.get(i))) {
                        return false;
                    }
                }
    ...
    public String toString() {
        PotionType inputType = PotionUtils.getPotionFromItem(this.potionInput);
        PotionType outputType = PotionUtils.getPotionFromItem(this.potionOutput);
        return this.ingredients + " + [" + this.potionInput.getItem() + " " + inputType.getNamePrefixed("") + "] = [" + this.potionOutput + " " + outputType.getNamePrefixed("") + "]";
    }
}
